...
fun shortenEndpointId(endpointId: String): String =
    "${endpointId.slice(0..<6)}...${endpointId.slice((endpointId.length - 6)..<(endpointId.length))}"
...
fun formatSize(
    ...
    formatSize(
        ...
        estimated = fileSizeModel is FileSizeModel.Estimated,
        decimals = decimals,
    )
...
fun mockClientModel(
    transferJobs: List<TransferJobModel> =
        buildList {
            repeat(100) {
...
fun mockTransferJobProgressModelInProgress(bytes: ULong = 2345678u) =
    TransferJobProgressModel.InProgress(
        startedAt = now() - 5u,
        bytes = CounterModel(bytes),
    )

fun mockTransferJobProgressModelFinished() =
    TransferJobProgressModel.Finished(
        finishedAt = now() - 1u,
    )

fun mockTransferJobProgressModelFailed() =
...
fun mockLibraryModel(
    ...
    LibraryModel(
        ...
        transcodesDirSize =
            if (cachedTranscodes) {
                FileSizeModel.Actual(534_000_000uL)
            } else {
                FileSizeModel.Actual(
                    0uL,
                )
            },
